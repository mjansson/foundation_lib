...
int hashify_process_strings( const char* const* strings )
{
	...
	for( istr = 0, strings_size = array_size( strings ); istr < strings_size; ++istr )
	{
		...
		log_infof( 0, "String '%s' hash: 0x%" PRIx64 "", strings[istr], hash_value );
	}
	return 0;
}
...
int hashify_process_file( stream_t* input_file, stream_t* output_file, bool check_only, hashify_string_t** history )
{
	...
	while( !stream_eos( input_file ) && ( result == HASHIFY_RESULT_OK ) )
	{
		...
		if( string_length( def_string ) )
		{
			...
			log_infof( 0, "  %s: %s -> 0x%" PRIx64 "", def_string, value_string, hash_value );
	...
	do
	{
		...
		if( ( string_find_string( line, "define", 0 ) != STRING_NPOS ) && ( string_find_string( line, "static_hash", 0 ) != STRING_NPOS ) )
		{
			...
			if( array_size( tokens ) >= 6 )
			{
				...
				{
					log_errorf( 0, ERROR_INVALID_VALUE, "  hash output file is out of date, %s is set to 0x%" PRIx64 " but should be 0x%" PRIx64 " ", hash_string.string, hash_string.hash, hash( hash_string.string, string_length( hash_string.string ) ) );
...
int hashify_write_file( stream_t* generated_file, const char* output_filename )
{
	...
	if( need_update )
	{
		...
		if( result == HASHIFY_RESULT_OK )
		{
			...
			log_infof( 0, "  wrote %s : %" PRIu64 " bytes", output_filename, total_written );
		}
	}
	else
	{
...
int hashify_check_local_consistency( const char* string, hash_t hash_value, const hashify_string_t* local_hashes )
{
	...
	for( ilocal = 0, localsize = array_size( local_hashes ); ilocal < localsize; ++ilocal )
	{
		if( local_hashes[ilocal].hash == hash_value )
		{
			...
			{
				log_errorf( 0, ERROR_INVALID_VALUE, "  hash string mismatch, \"%s\" with hash 0x%" PRIx64 " stored in output file, read \"%s\" from input file", local_hashes[ilocal].string, local_hashes[ilocal].hash, string );
		...
		{
			log_errorf( 0, ERROR_INVALID_VALUE, "  hash mismatch, \"%s\" with hash 0x%" PRIx64 " stored in output file, read \"%s\" with hash 0x%" PRIx64 " from input file", local_hashes[ilocal].string, local_hashes[ilocal].hash, string, hash_value );
			return HASHIFY_RESULT_HASH_MISMATCH;
		}
	}
	if( ilocal == localsize )
	{
		log_errorf( 0, ERROR_INVALID_VALUE, "  hash missing in output file, \"%s\" with hash 0x%" PRIx64 " ", string, hash_value );
...
int hashify_check_collisions( const char* string, hash_t hash_value, const hashify_string_t* history )
{
	...
	for( ihist = 0, history_size = array_size( history ); ihist < history_size; ++ihist )
	{
		if( history[ihist].hash == hash_value )
		{
			...
			{
				log_errorf( 0, ERROR_INVALID_VALUE, "  global hash collision, 0x%" PRIx64 " between: \"%s\" and \"%s\" ", hash_value, string, history[ihist].string );
	...
	for( ihash = 0, hashes_size = array_size( hashes ); ihash < hashes_size; ++ihash )
	{
		for( igen = 0, generated_size = array_size( generated ); igen < generated_size; ++igen )
		{
			if( hashes[ihash].hash == generated[igen].hash )
			{
				...
				{
					log_errorf( 0, ERROR_INVALID_VALUE, "  hash string mismatch, \"%s\" with hash 0x%" PRIx64 " stored in output file, generated by \"%s\" from input file", hashes[ihash].string, hashes[ihash].hash, generated[igen].string );
					return HASHIFY_RESULT_HASH_STRING_MISMATCH;
				}
				break;
			}
			...
			{
				log_errorf( 0, ERROR_INVALID_VALUE, "  hash mismatch, \"%s\" with hash 0x%" PRIx64 " stored in output file, \"%s\" generated hash 0x%" PRIx64 " from input file", hashes[ihash].string, hashes[ihash].hash, generated[igen].string, generated[igen].hash );
				return HASHIFY_RESULT_HASH_MISMATCH;
			}
		}
		if( igen == generated_size )
		{
			log_errorf( 0, ERROR_INVALID_VALUE, "  extra hash \"%s\" with hash 0x%" PRIx64 " not found in input file", hashes[ihash].string, hashes[ihash].hash );
