...
#define EXPECT_REALZERO( var ) do { if( !math_realiszero(var) ) { log_warnf( HASH_TEST, WARNING_SUSPICIOUS, STRING_CONST( "Test failed, %s is not zero real (at %s:%u): %.6" PRIREAL ), FOUNDATION_PREPROCESSOR_TOSTRING(var), __FILE__, __LINE__, (real)((var)) ); RETURN_FAILED_TEST; } } while(0)
#define EXPECT_REALONE( var ) do { if( !math_realisone(var) ) { log_warnf( HASH_TEST, WARNING_SUSPICIOUS, STRING_CONST( "Test failed, %s is not one real (at %s:%u): %.6" PRIREAL ), FOUNDATION_PREPROCESSOR_TOSTRING(var), __FILE__, __LINE__, (real)((var)) ); RETURN_FAILED_TEST; } } while(0)
