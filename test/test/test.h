...
#define EXPECT_STREQ( var, varlen, expect, expectlen ) do { if( !string_equal( (var), (varlen), (expect), (expectlen) ) ) { log_warnf( HASH_TEST, WARNING_SUSPICIOUS, STRING_CONST( "Test failed, %s != %s ('%.*s', '%.*s') (at %s:%u)" ), FOUNDATION_PREPROCESSOR_TOSTRING(var), FOUNDATION_PREPROCESSOR_TOSTRING(expect), (int)(varlen), (var), (int)(expectlen), (expect), __FILE__, __LINE__ ); RETURN_FAILED_TEST; } } while(0)
#define EXPECT_STREQ_MSG( var, varlen, expect, expectlen, msg ) do { if( !string_equal( (var), (varlen), (expect), (expectlen) ) ) { log_warnf( HASH_TEST, WARNING_SUSPICIOUS, STRING_CONST( "Test failed, %s != %s ('%s', '%s') (at %s:%u) : %s" ), FOUNDATION_PREPROCESSOR_TOSTRING(var), FOUNDATION_PREPROCESSOR_TOSTRING(expect), (int)(varlen), (var), (int)(expectlen), (expect), __FILE__, __LINE__, msg ); RETURN_FAILED_TEST; } } while(0)
#define EXPECT_STREQ_MSGFORMAT( var, varlen, expect, expectlen, format, ... ) do { if( !string_equal( (var), (varlen), (expect), (expectlen) ) ) { log_warnf( HASH_TEST, WARNING_SUSPICIOUS, STRING_CONST( "Test failed, %s != %s ('%s', '%s') (at %s:%u) : " format ), FOUNDATION_PREPROCESSOR_TOSTRING(var), FOUNDATION_PREPROCESSOR_TOSTRING(expect), (int)(varlen), (var), (int)(expectlen), (expect), __FILE__, __LINE__, __VA_ARGS__ ); RETURN_FAILED_TEST; } } while(0)

#define EXPECT_STRINGEQ( var, expect ) do { string_const_t lhs = string_const( STRING_ARGS( (var) ) ); string_const_t rhs = string_const( STRING_ARGS( (expect) ) ); if( !string_equal( STRING_ARGS( lhs ), STRING_ARGS( rhs ) ) ) { log_warnf( HASH_TEST, WARNING_SUSPICIOUS, STRING_CONST( "Test failed, %s != %s ('%.*s', '%.*s') (at %s:%u)" ), FOUNDATION_PREPROCESSOR_TOSTRING(var), FOUNDATION_PREPROCESSOR_TOSTRING(expect), STRING_FORMAT( lhs ), STRING_FORMAT( rhs ), __FILE__, __LINE__ ); RETURN_FAILED_TEST; } } while(0)
#define EXPECT_STRINGEQ_MSG( var, expect, msg ) do { string_const_t lhs = string_const( STRING_ARGS( (var) ) ); string_const_t rhs = string_const( STRING_ARGS( (expect) ) ); if( !string_equal( STRING_ARGS( (var) ), STRING_ARGS( (expect) ) ) ) { log_warnf( HASH_TEST, WARNING_SUSPICIOUS, STRING_CONST( "Test failed, %s != %s ('%s', '%s') (at %s:%u) : %s" ), FOUNDATION_PREPROCESSOR_TOSTRING(var), FOUNDATION_PREPROCESSOR_TOSTRING(expect), STRING_FORMAT( lhs ), STRING_FORMAT( rhs ), __FILE__, __LINE__, msg ); RETURN_FAILED_TEST; } } while(0)
#define EXPECT_STRINGEQ_MSGFORMAT( var, expect, format, ... ) do { string_const_t lhs = string_const( STRING_ARGS( (var) ) ); string_const_t rhs = string_const( STRING_ARGS( (expect) ) ); if( !string_equal( STRING_ARGS( (var) ), STRING_ARGS( (expect) ) ) ) { log_warnf( HASH_TEST, WARNING_SUSPICIOUS, STRING_CONST( "Test failed, %s != %s ('%s', '%s') (at %s:%u) : " format ), FOUNDATION_PREPROCESSOR_TOSTRING(var), FOUNDATION_PREPROCESSOR_TOSTRING(expect), STRING_FORMAT( lhs ), STRING_FORMAT( rhs ), __FILE__, __LINE__, __VA_ARGS__ ); RETURN_FAILED_TEST; } } while(0)

#define EXPECT_STRINGNE( var, expect ) do { string_const_t lhs = string_const( STRING_ARGS( (var) ) ); string_const_t rhs = string_const( STRING_ARGS( (expect) ) ); if( string_equal( STRING_ARGS( (var) ), STRING_ARGS( (expect) ) ) ) { log_warnf( HASH_TEST, WARNING_SUSPICIOUS, STRING_CONST( "Test failed, %s == %s ('%.*s', '%.*s') (at %s:%u)" ), FOUNDATION_PREPROCESSOR_TOSTRING(var), FOUNDATION_PREPROCESSOR_TOSTRING(expect), STRING_FORMAT( lhs ), STRING_FORMAT( rhs ), __FILE__, __LINE__ ); RETURN_FAILED_TEST; } } while(0)
#define EXPECT_STRINGNE_MSG( var, expect, msg ) do { string_const_t lhs = string_const( STRING_ARGS( (var) ) ); string_const_t rhs = string_const( STRING_ARGS( (expect) ) ); if( string_equal( STRING_ARGS( lhs ), STRING_ARGS( rhs ) ) ) { log_warnf( HASH_TEST, WARNING_SUSPICIOUS, STRING_CONST( "Test failed, %s == %s ('%s', '%s') (at %s:%u) : %s" ), FOUNDATION_PREPROCESSOR_TOSTRING(var), FOUNDATION_PREPROCESSOR_TOSTRING(expect), STRING_FORMAT( lhs ), STRING_FORMAT( rhs ), __FILE__, __LINE__, msg ); RETURN_FAILED_TEST; } } while(0)
#define EXPECT_STRINGNE_MSGFORMAT( var, expect, format, ... ) do { string_const_t lhs = string_const( STRING_ARGS( (var) ) ); string_const_t rhs = string_const( STRING_ARGS( (expect) ) ); if( string_equal( STRING_ARGS( (var) ), STRING_ARGS( (expect) ) ) ) { log_warnf( HASH_TEST, WARNING_SUSPICIOUS, STRING_CONST( "Test failed, %s == %s ('%s', '%s') (at %s:%u) : " format ), FOUNDATION_PREPROCESSOR_TOSTRING(var), FOUNDATION_PREPROCESSOR_TOSTRING(expect), STRING_FORMAT( lhs ), STRING_FORMAT( rhs ), __FILE__, __LINE__, __VA_ARGS__ ); RETURN_FAILED_TEST; } } while(0)
