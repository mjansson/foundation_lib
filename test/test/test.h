...
#define EXPECT_REALZERO( var ) do { if( !math_realzero(var) ) { log_warnf( WARNING_SUSPICIOUS, "Test failed, %s is not zero real (at %s:%u)", FOUNDATION_PREPROCESSOR_TOSTRING(var), __FILE__, __LINE__ ); return FAILED_TEST; } } while(0)
#define EXPECT_REALONE( var ) do { if( !math_realone(var) ) { log_warnf( WARNING_SUSPICIOUS, "Test failed, %s is not one real (at %s:%u)", FOUNDATION_PREPROCESSOR_TOSTRING(var), __FILE__, __LINE__ ); return FAILED_TEST; } } while(0)
#define EXPECT_REALNE( var, expect ) do { if( math_realeq( (var), (expect), 10 ) ) { log_warnf( WARNING_SUSPICIOUS, "Test failed, %s == %s real (at %s:%u)", FOUNDATION_PREPROCESSOR_TOSTRING(var), FOUNDATION_PREPROCESSOR_TOSTRING(expect), __FILE__, __LINE__ ); return FAILED_TEST; } } while(0)
#define EXPECT_REALEQ( var, expect ) do { if( !math_realeq( (var), (expect), 10 ) ) { log_warnf( WARNING_SUSPICIOUS, "Test failed, %s != %s real (at %s:%u)", FOUNDATION_PREPROCESSOR_TOSTRING(var), FOUNDATION_PREPROCESSOR_TOSTRING(expect), __FILE__, __LINE__ ); return FAILED_TEST; } } while(0)
#define EXPECT_REALEQULPS( var, expect, ulps ) do { if( !math_realeq( (var), (expect), (ulps) ) ) { log_warnf( WARNING_SUSPICIOUS, "Test failed, %s != %s real (at %s:%u)", FOUNDATION_PREPROCESSOR_TOSTRING(var), FOUNDATION_PREPROCESSOR_TOSTRING(expect), __FILE__, __LINE__ ); return FAILED_TEST; } } while(0)
