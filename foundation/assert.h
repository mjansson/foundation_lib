...
#  define FOUNDATION_ASSERT_FAIL_LOG( context, msg ) /*lint -save -e717 */ do { if( assert_report( 0, __FILE__, __LINE__, (msg) ) ) FOUNDATION_BREAKPOINT(); (void)sizeof( context ); } while(0) /*lint -restore */
...
#  define FOUNDATION_ASSERT_FAILFORMAT_LOG( context, msg, ... ) /*lint -save -e717 */ do { if( assert_report_formatted( 0, __FILE__, __LINE__, (msg), __VA_ARGS__ ) ) FOUNDATION_BREAKPOINT(); (void)sizeof( context ); } while(0) /*lint -restore */
...
#  define FOUNDATION_ASSERT_FAIL_LOG( context, msg ) /*lint -save -e717 */ do { log_errorf( context, ERROR_ASSERT, "%s", msg ); } while(0) /*lint -restore */
#  define FOUNDATION_ASSERT_FAILFORMAT( msg, ... ) /*lint -save -e717 */ do { (void)sizeof( msg ); } while(0) /*lint -restore */
#  define FOUNDATION_ASSERT_FAILFORMAT_LOG( context, msg, ... ) /*lint -save -e717 */ do { log_errorf( context, ERROR_ASSERT, msg, __VA_ARGS__ ); } while(0) /*lint -restore */
