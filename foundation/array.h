...
#define array_insert_safe( array, pos, element ) do { \
  ...
  uint32_t _clamped_pos = ((uint32_t)(pos) & 0x80000000U) ? \
    0 : math_min((uint32_t)(pos), _arr_size); \
...
#define array_erase_ordered_range_safe(array, pos, num) do { \
  ...
  uint32_t _clamped_start = ((uint32_t)(pos) & 0x80000000U) ? \
    0 : math_min((uint32_t)(pos), _arr_size); \
  uint32_t _clamped_end = ((uint32_t)((pos) + (num)) & 0x80000000U) ? \
    0 : math_min((uint32_t)((pos) + (num)), _arr_size); \
...
/*! Access the raw array header area */
...
Array memory allocation function. This will reallocate array storage with the given
parameters, resulting in a total size of (factor * previous_capacity + increment)
elements, with each element size given by itemsize.
...
FOUNDATION_API void*
_array_growfn(void** arr, size_t increment, size_t factor, size_t itemsize);
...
\return         New array pointer */
FOUNDATION_API void*
_array_resizefn(void** arr, size_t elements, size_t itemsize);
