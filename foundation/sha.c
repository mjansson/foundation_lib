...
static uint32_t choise32(uint32_t x, uint32_t y, uint32_t z) { return z ^ (x & (y ^ z)); }
static uint32_t majority32(uint32_t x, uint32_t y, uint32_t z) { return ((x | y) & z) | (x & y); }
static uint32_t rotate32(uint32_t x, uint32_t n) { return (x >> (n & 31)) | (x << (32 - (n & 31))); }
static uint32_t shift32(uint32_t x, uint32_t n) { return x >> n; }
static uint32_t sigma0_32(uint32_t x) { return rotate32(x, 2) ^ rotate32(x, 13) ^ rotate32(x, 22); }
static uint32_t sigma1_32(uint32_t x) { return rotate32(x, 6) ^ rotate32(x, 11) ^ rotate32(x, 25); }
static uint32_t gamma0_32(uint32_t x) { return rotate32(x, 7) ^ rotate32(x, 18) ^ shift32(x, 3); }
static uint32_t gamma1_32(uint32_t x) { return rotate32(x, 17) ^ rotate32(x, 19) ^ shift32(x, 10); }

static uint64_t choise64(uint64_t x, uint64_t y, uint64_t z) { return z ^ (x & (y ^ z)); }
static uint64_t majority64(uint64_t x, uint64_t y, uint64_t z) { return ((x | y) & z) | (x & y); }
static uint64_t rotate64(uint64_t x, uint64_t n) { return (x >> (n & 63)) | (x << (64 - (n & 63))); }
static uint64_t shift64(uint64_t x, uint64_t n) { return x >> n; }
static uint64_t sigma0_64(uint64_t x) { return rotate64(x, 28) ^ rotate64(x, 34) ^ rotate64(x, 39); }
static uint64_t sigma1_64(uint64_t x) { return rotate64(x, 14) ^ rotate64(x, 18) ^ rotate64(x, 41); }
static uint64_t gamma0_64(uint64_t x) { return rotate64(x, 1) ^ rotate64(x, 8) ^ shift64(x, 7); }
static uint64_t gamma1_64(uint64_t x) { return rotate64(x, 19) ^ rotate64(x, 61) ^ shift64(x, 6); }

static void compress32(const uint32_t* wbox,
...
static void compress64(const uint64_t* wbox,
...
static void sha256_compress(sha256_t* digest, const unsigned char* buffer) {
	...
	for (i = 0; i < 64;) {
		...
		compress32(wbox, sbox[7], sbox[0], sbox[1], sbox + 2,
		           sbox[3], sbox[4], sbox[5], sbox + 6, i++);

		compress32(wbox, sbox[6], sbox[7], sbox[0], sbox + 1,
		           sbox[2], sbox[3], sbox[4], sbox + 5, i++);

		compress32(wbox, sbox[5], sbox[6], sbox[7], sbox + 0,
		           sbox[1], sbox[2], sbox[3], sbox + 4, i++);

		compress32(wbox, sbox[4], sbox[5], sbox[6], sbox + 7,
		           sbox[0], sbox[1], sbox[2], sbox + 3, i++);

		compress32(wbox, sbox[3], sbox[4], sbox[5], sbox + 6,
		           sbox[7], sbox[0], sbox[1], sbox + 2, i++);

		compress32(wbox, sbox[2], sbox[3], sbox[4], sbox + 5,
		           sbox[6], sbox[7], sbox[0], sbox + 1, i++);

		compress32(wbox, sbox[1], sbox[2], sbox[3], sbox + 4,
...
static void sha512_compress(sha512_t* digest, const unsigned char* buffer) {
	...
	for (i = 0; i < 80;) {
		...
		compress64(wbox, sbox[7], sbox[0], sbox[1], sbox + 2,
		           sbox[3], sbox[4], sbox[5], sbox + 6, i++);

		compress64(wbox, sbox[6], sbox[7], sbox[0], sbox + 1,
		           sbox[2], sbox[3], sbox[4], sbox + 5, i++);

		compress64(wbox, sbox[5], sbox[6], sbox[7], sbox + 0,
		           sbox[1], sbox[2], sbox[3], sbox + 4, i++);

		compress64(wbox, sbox[4], sbox[5], sbox[6], sbox + 7,
		           sbox[0], sbox[1], sbox[2], sbox + 3, i++);

		compress64(wbox, sbox[3], sbox[4], sbox[5], sbox + 6,
		           sbox[7], sbox[0], sbox[1], sbox + 2, i++);

		compress64(wbox, sbox[2], sbox[3], sbox[4], sbox + 5,
		           sbox[6], sbox[7], sbox[0], sbox + 1, i++);

		compress64(wbox, sbox[1], sbox[2], sbox[3], sbox + 4,
		           sbox[5], sbox[6], sbox[7], sbox + 0, i++);
	}
