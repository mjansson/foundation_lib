...
ifeq ($(BUILD_DEPLOY),1)
LOCAL_CFLAGS     += -DNDEBUG=1 -DBUILD_DEPLOY=1 -O3
TARGET_OBJS      := $(TARGET_OUT)/$(LOCAL_MODULE)-deploy
else
ifeq ($(BUILD_PROFILE),1)
LOCAL_CFLAGS     += -DNDEBUG=1 -DBUILD_PROFILE=1 -O3
TARGET_OBJS      := $(TARGET_OUT)/$(LOCAL_MODULE)-profile
else
LOCAL_CFLAGS     += -DNDEBUG=1 -DBUILD_RELEASE=1 -O3
